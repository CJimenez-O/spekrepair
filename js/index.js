...
gsap.from(".hero-content h3", { opacity: 0, duration: 1, delay: 2, y: -50 });
gsap.from(".hero-content h2", { opacity: 0, duration: 1, delay: 3, y: -50 });
...
function displayCart() {
    ...
    if( cartItems && productContainer ) {
        ...
        Object.values(cartItems).map( (item, index) => {
            productContainer.innerHTML += 
            `<div class="cart-product"><ion-icon name="close-circle"></ion-icon><img src="./images/${item.tag}.png" />
                <span class="sm-hide">${item.name}</span>
            </div>
            <div class="cart-price sm-hide">$${item.price}</div>
            <div class="quantity">
...
function manageQuantity() {
    ...
    for(let i=0; i < increaseButtons.length; i++) {
        ...
        increaseButtons[i].addEventListener('click', () => {
            ...
            console.log('prod', cartItems['iPhone12']);
            cartItems[currentProduct].inCart += 1;
